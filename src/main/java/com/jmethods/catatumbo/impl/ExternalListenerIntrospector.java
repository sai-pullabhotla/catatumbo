...
public class ExternalListenerIntrospector {
	...
	private void introspect() {
		if (!listenerClass.isAnnotationPresent(EntityListener.class)) {
			String message = String.format("Class %s must have %s annotation to be used as an EntityListener",
	...
	/**
	 * Processes the methods in the listener class and updates the metadata for
	 * any valid methods found.
	 */
	...
	/**
	 * Validates the given method to ensure if it is a valid callback method for
	...
	private void validateMethod(Method method, CallbackType callbackType) {
		...
		if (parameters.length != 1) {
			String message = String.format(
					"Method %s in class %s is not a valid %s callback method. Method must have one parameter. ",
					method.getName(), method.getDeclaringClass().getName(), callbackType);
			throw new EntityManagerException(message);
		}
