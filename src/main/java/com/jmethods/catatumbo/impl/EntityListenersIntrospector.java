...
public class EntityListenersIntrospector {
	...
	private List<Class<?>> getAllExternalListeners() {
		...
		while (!stop) {
			...
			boolean excludeSuperClassListeners = clazz.isAnnotationPresent(ExcludeSuperclassListeners.class);
			...
			stop = excludeSuperClassListeners || clazz == null || !clazz.isAnnotationPresent(MappedSuperClass.class);
		}
		return allListeners;
	}

	/**
	 * Introspects the given listener class and finds all methods that should
	 * receive callback event notifications.
	 * 
	 * @param listenerClass
	 *            the listener class
	 */
	...
	private void validateExternalCallback(Method method, CallbackType callbackType) {
		...
		if (!parameters[0].isAssignableFrom(entityClass)) {
			String message = String.format("Method %s in class %s is not valid for entity %s", method.getName(),
					method.getDeclaringClass().getName(), entityClass.getName());
			throw new EntityManagerException(message);
		}
		CallbackMetadata callbackMetadata = new CallbackMetadata(EntityListenerType.EXTERNAL, callbackType, method);
		metadata.put(callbackType, callbackMetadata);
	}

	/**
	 * Introspects the entity class hierarchy for any internal callback methods
	...
	/**
	 * Traverses up the entity class hierarchy and returns the list of all
	 * classes that may potentially have callback listeners.
	 * 
